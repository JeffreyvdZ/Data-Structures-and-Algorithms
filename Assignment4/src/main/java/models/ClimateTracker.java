...
public class ClimateTracker {
    ...
    //return a LinkedHashSet so the order of the stations remain the same as in the map.
    public Set<Station> getStations() {
        return new LinkedHashSet<>(stations.values());
    }

    // return the station we need to find by inserting the key into the stations map.
    ...
    public Map<Station, Integer> numberOfMeasurementsByStation() {
        ...
        // Stream through the stations and collect this into a map where the station is the key
        // We use a linkedhashmap so the order of the items remain the same.
        // And we get the size of the measurements that belongs to that station.
        return getStations()
                .stream()
                .collect(Collectors.groupingBy(station -> station, LinkedHashMap::new, Collectors.summingInt(station -> station.getMeasurements().size())));
    }
    /**
    ...
    public Map<Station, LocalDate> firstDayOfMeasurementByStation() {
        ...
        Map<Station, LocalDate> stationLocalDateMap = new LinkedHashMap<>();
        // for each station put it inside the map if the first day of measurement is valid.
        getStations().forEach(s -> s.firstDayOfMeasurement().ifPresent(x -> stationLocalDateMap.put(s, x)));
        return stationLocalDateMap;
    }
    /**
    ...
    public Map<Station, Integer> numberOfValidValuesByStation(Function<Measurement, Double> mapper) {
        ...
        // stream through the stations and collect this to a map where every station becomes a key in a linked hash map
        // with its value being the number of valid values for the specified quantity using the helper method of station and the mapper.
        return getStations()
                .stream()
                .collect(Collectors.groupingBy(station -> station, LinkedHashMap::new, Collectors.summingInt(station -> station.numValidValues(mapper))));
    }
    /**
     * Calculates for each calendar year in the dataset the average daily temperatures(TG)
    ...
    public Map<Integer, Double> annualAverageTemperatureTrend() {
        ...
        // First we stream through the stations and map this so that we get from every station it's measurements.
        // Then we add these measurements from the separate Stations to one collection stream using flatmap.
        // Now that we have all measurements of all stations in one stream we can filter so that we only have data of the average daily temperature, and we remove all invalid values
        // Afterwards we can transfer this data to a map where the key is the year of the measurement, and it's value is the average temperature across all days in the year and all stations in this tracker
        return getStations()
                .stream()
                .map(Station::getMeasurements).flatMap(Collection::stream)
                .filter(measurement -> !Double.isNaN(measurement.getAverageTemperature()))
                .collect(Collectors.groupingBy(measurement -> measurement.getDate().getYear(), Collectors.averagingDouble(Measurement::getAverageTemperature)));
    }
    /**
    ...
    public Map<Integer, Double> annualMaximumTrend(Function<Measurement, Double> mapper) {
        ...
        // First we stream through the stations and map this so that we get from every station it's measurements.
        // Then we add these measurements from the separate Stations to one collection stream using flatmap.
        // Now that we have all measurements of all stations in one stream we can filter so that we only have data of the required datafield using the mapper in the parameter.
        // Where we also check if the value is valid or not
        // Afterwards we can transfer this data to a map where the key is the year and the value is the maximum value in that year for the required datafield of the mapper.
        return getStations()
                .stream()
                .map(Station::getMeasurements)
                .flatMap(Collection::stream)
                .filter(measurement -> !Double.isNaN(mapper.apply(measurement)))
                .collect(Collectors.toMap(measurement -> measurement.getDate().getYear(), mapper, Double::max));
    }
    /**
    ...
    public Map<Month, Double> allTimeAverageDailySolarByMonth() {
        ...
        // First we stream through the stations and map this so that we get from every station it's measurements.
        // Then we add these measurements from the separate Stations to one collection stream using flatmap.
        //  Now that we have all measurements of all stations in one stream we can filter so that we only have data of the solar hours, and we remove all invalid values
        // Afterwards we can transfer this data to a map where the key is the month and the value is the average daily hours of sunshine across all years and all stations for that particular month.
        return getStations()
                .stream()
                .map(Station::getMeasurements)
                .flatMap(Collection::stream)
                .filter(measurement -> !Double.isNaN(measurement.getSolarHours()))
                .collect(Collectors.groupingBy(measurement -> measurement.getDate().getMonth(), LinkedHashMap::new, Collectors.averagingDouble(Measurement::getSolarHours)));
    }
    /**
     ...
     * I.e. any daily value of a minimum temperature(TN) at a station that is above zero should not be included in its sum for the year
     * The lowest yearsum of negative minimum temperatures(TN) indicates the coldest year.
     *
    ...
    public int coldestYear() {
        ...
        // First we stream through the stations and map this so that we get from every station it's measurements.
        // Then we add these measurements from the separate Stations to one collection stream using flatmap.
        //  Now that we have all measurements of all stations in one stream we can filter so that we only have data of the mintemperatures below 0.
        // Afterwards we can turn this to a map where the key is the year and the value is the sum of all negative mintemperatures in that year.
        // Now we can stream through this map we just made and compare the values of all keys and return the key that is the coldest year or return -1.
        return getStations()
                .stream()
                .map(Station::getMeasurements)
                .flatMap(Collection::stream)
                .filter(measurement -> measurement.getMinTemperature()< 0)
                .collect(Collectors.groupingBy(measurement -> measurement.getDate().getYear(), Collectors.summingDouble(Measurement::getMinTemperature)))
                .entrySet()
                .stream()
                .min(Map.Entry.comparingByValue())
                .orElse(Map.entry(-1, 0.0))
                .getKey();
    }
    /**
    ...
    /**
     * Helper method used for personal test purposes.
     * @param stations
     */
